          ...
          if use_sse2
              then do
               ...
               then return $ toOL [
                  ...
                  (MOV (intFormat w) (OpReg eax) (OpReg r_dest))
          ...
          let sz = cTypeSize i
              ...
              code =
                if isFloatType ty && not use_sse2
                  then unitOL (GLD (cmmTypeFormat rep) addr r_dest)
                  else if isWord64 ty
                         then toOL [
                           MOV II32 (OpAddr addr) (OpReg r_dest_hi),
                           MOV II32 (OpAddr addr_lw) (OpReg r_dest)
                           ]
                         else unitOL (MOV (cmmTypeFormat rep)
                                      (OpAddr addr)
                                      (OpReg r_dest))
  ...
  assign_quartet (result, (reg:regs)) dest_regs = do
    ...
    assign_dest use_sse2 ((rep, dest), pos) = unitOL $
      ...
      in if isFloatType ty && not use_sse2
           then GLD (cmmTypeFormat rep) addr r_dest
           else MOV (cmmTypeFormat rep) (OpAddr addr) (OpReg r_dest)
...
assign_code64 dflags (Just (CPrimType _)) [dest] = return $
  if isFloatType rep
    then unitOL $ MOV (floatFormat $ typeWidth rep) (OpReg xmm0) (OpReg r_dest)
    else unitOL $ MOV (cmmTypeFormat rep) (OpReg rax) (OpReg r_dest)
  where
  platform = targetPlatform dflags
  ...
  assign_octet' reg dest_regs = do
    ...
    move_to_stack = do
      ...
      return $ toOL [
        ...
        MOV (intFormat $ wordWidth dflags)
            (OpReg reg)
